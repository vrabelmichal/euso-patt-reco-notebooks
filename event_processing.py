...
def gray_hough_line(image, size=2, phi_range=np.linspace(0, np.pi, 100), rho_step=1):
...
def select_neighbours(initial_seed_points, image, max_gap=3, val_ratio_thr=.85):
        ...
        for i in range(i_start, i_end):
            for j in range(j_start, j_end):
                if i == seed_i and j == seed_j:
                    continue
                if not out_neighbourhood[i,j]:
                    # todo add option to select only neighbours of initial seeds
                    if image[i,j]/image[seed_i, seed_j] > val_ratio_thr and (i,j) not in seed_points:
    ...
    return out_neighbourhood, grouped_neighbourhoods
...
def select_trigger_groups(trigger_points, image, max_gap=3):
    # seed_points iterable of pairs
    # presuming 2d matrix
    ...
    for trigger_point in trigger_points:
        for c_trigger_point in trigger_points:
            if c_trigger_point != trigger_point and \
                    ...
                    abs(trigger_point[1] - c_trigger_point[0]) <= max_gap:
...
def process_event(frames, exp_tree, pixels_mask = None):
    ...
    # trigger_neighbours, trigg_groups = select_neighbours(triggered_pixels_coords,max_values_arr)
    ...
    # gray_hough_line(max_values_arr_trigg)
