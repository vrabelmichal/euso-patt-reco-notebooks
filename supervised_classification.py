...
def draw_distributions(query_format, con, num_at_once=100000, max_rows=1000000,
                       drawn_columns=None, columns_offset=None, num_columns_at_once=None):
    ...
    last_col = (first_col + num_columns_at_once) if num_columns_at_once is not None else None

    out = []

    row = array.array('f',[0.]*len(all_columns))

    while first_col < len(all_columns):
        columns = all_columns[first_col:last_col]

        num_figures = len(columns)
